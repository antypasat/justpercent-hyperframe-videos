...
// Run: node record.mjs [theme] [fps]   → out/neuro-match-cut-<theme>.mp4
...
const ff = spawn("/opt/homebrew/bin/ffmpeg", [
  ...
  "-movflags", "+faststart",
  join(ROOT, "out", `neuro-match-cut-${theme}.mp4`)
...
for (let i = 0; i < frames; i++) {
  const t = i / fps;
  await page.evaluate(
    async ({ t, theme }) => window.__f5Seek(t, theme), { t, theme }
  );
...
await browser.close();
console.log(`out/neuro-match-cut-${theme}.mp4 done`);
