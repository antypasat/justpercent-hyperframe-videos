...
// Run: node record.mjs [theme] [fps]   → out/gaussian-splat-morph-<theme>.mp4
...
const ff = spawn("/opt/homebrew/bin/ffmpeg", [
  ...
  "-movflags", "+faststart",
  join(ROOT, "out", `gaussian-splat-morph-${theme}.mp4`)
...
await page.goto(
  `file://${join(ROOT, "gaussian-splat-morph.html")}?rec&pause&theme=${theme}`,
  { waitUntil: "load" }
);
...
for (let i = 0; i < frames; i++) {
  const t = i / fps;
  await page.evaluate(
    async ({ t, theme }) => window.__f6Seek(t, theme), { t, theme }
  );
...
await browser.close();
console.log(`out/gaussian-splat-morph-${theme}.mp4 done`);
