...
for (const t of T) {
  await page.evaluate(async ({ t, theme }) => window.__f3Seek(t, theme), { t, theme });
  await page.waitForTimeout(140);
  await page.screenshot({
    path: join(ROOT, "qa", `f3-${theme}-${t.toFixed(2)}s.png`)
  });
  console.log(`qa f3-${theme}-${t.toFixed(2)}s.png`);
}
await browser.close();
