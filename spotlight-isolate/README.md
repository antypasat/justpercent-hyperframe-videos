...
| s3 | 0:22–0:31 | 3.7 Click-Reveal | Practical Uses pod kalkulatorem; klik w pin „Energy Savings" → crossfade do stanu pinned: autofill **200 / 18 → 164** + banner „Selected Practical Use Example" |
...
| s6 | 0:49–0:58 | 3.5 Vignette Focus | `/faqs/` (hub, 64 podstrony) z miękką winietą → `/faqs/compound-interest-calculator/` — winieta + glow na żywym wyniku **1157.63** |
...
* `https://justpercent.com/faqs/compound-interest-calculator/`
...
* `assets/logo-dataurl.json` — oryginalne logo `/images/percentage.webp` jako dataURL
  (ten sam dataURL jest wklejony inline jako `<img class="pctbadge">` w s0/s7 w `index.html`).
