...
// Run: node record.mjs [theme] [fps]   → out/saas-brain-rot-<theme>.mp4
...
const ff = spawn("/opt/homebrew/bin/ffmpeg", [
  ...
  "-movflags", "+faststart",
  join(ROOT, "out", `saas-brain-rot-${theme}.mp4`)
...
for (let i = 0; i < frames; i++) {
  const t = i / fps;
  await page.evaluate(
    async ({ t, theme }) => window.__f7Seek(t, theme), { t, theme }
  );
...
await browser.close();
console.log(`out/saas-brain-rot-${theme}.mp4 done`);
