...
for (const t of T) {
  await page.evaluate(async ({ t, theme }) => window.__f2Seek(t, theme), { t, theme });
  await page.waitForTimeout(120);
  await page.screenshot({
    path: join(ROOT, "qa", `f2-${theme}-${t.toFixed(2)}s.png`)
  });
  console.log(`qa f2-${theme}-${t.toFixed(2)}s.png`);
}
await browser.close();
