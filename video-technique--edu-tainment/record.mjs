...
// Run: node record.mjs [theme] [fps]   → out/edu-tainment-<theme>.mp4
...
const DURATION = 40; // seconds (configs.js meta.duration)
...
const ff = spawn("/opt/homebrew/bin/ffmpeg", [
  ...
  "-movflags", "+faststart",
  join(ROOT, "out", `edu-tainment-${theme}.mp4`)
...
await browser.close();
console.log(`out/edu-tainment-${theme}.mp4 done`);
