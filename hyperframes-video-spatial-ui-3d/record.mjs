...
// Run: node record.mjs [theme] [fps]   → out/spatial-ui-3d-<theme>.mp4
...
const ff = spawn("ffmpeg", [
  ...
  "-movflags", "+faststart",
  join(ROOT, "out", `spatial-ui-3d-${theme}.mp4`)
...
for (let i = 0; i < frames; i++) {
  const t = i / fps;
  await page.evaluate(
    async ({ t, theme }) => window.__f1Seek(t, theme), { t, theme }
  );
...
await browser.close();
console.log(`out/spatial-ui-3d-${theme}.mp4 done`);
